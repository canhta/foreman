...
func TestNewAgentRunner_Builtin(t *testing.T) {
	...
	runner, err := NewAgentRunner(cfg, nil, &mockSingleShotLLM{response: "ok"}, "test-model", nil, models.LLMConfig{}, nil, nil)
	if err != nil {
...
func TestNewAgentRunner_ClaudeCode(t *testing.T) {
	...
	runner, err := NewAgentRunner(cfg, &mockCmdRunner{exitCode: 0}, nil, "", nil, models.LLMConfig{}, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNewAgentRunner_Unknown(t *testing.T) {
	...
	_, err := NewAgentRunner(cfg, nil, nil, "", nil, models.LLMConfig{}, nil, nil)
	if err == nil {
