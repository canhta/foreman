<script lang="ts">
  ...
  let doneToday = $derived(
    appState.tickets.filter(t => DONE_STATUSES.includes(t.Status) && t.CreatedAt?.slice(0, 10) === new Date().toISOString().slice(0, 10)).length
  );
  let failedCount = $derived(appState.tickets.filter(t => FAIL_STATUSES.includes(t.Status)).length);
  let successRate = $derived(
    appState.tickets.length > 0 ? Math.round((appState.tickets.filter(t => DONE_STATUSES.includes(t.Status)).length / appState.tickets.length) * 100) : 0
  );
...
<div class="p-4 space-y-4 overflow-y-auto h-full">
  ...
  <section class="border border-border p-3 space-y-2">
    ...
    <div class="flex items-center gap-2 text-sm">
      <span class="w-2 h-2 rounded-full {appState.daemonState === 'running' ? 'bg-success' : 'bg-warning'}"></span>
      <span>{appState.daemonState.toUpperCase()}</span>
      <span class="text-muted">|</span>
      <span class="w-2 h-2 rounded-full {appState.wsConnected ? 'bg-success' : 'bg-danger'}"></span>
      <span class="text-xs text-muted">WebSocket {appState.wsConnected ? 'connected' : 'disconnected'}</span>
    </div>
    {#if appState.whatsapp !== null}
      <div class="flex items-center gap-2 text-xs">
        <span class="w-2 h-2 rounded-full {appState.whatsapp ? 'bg-success' : 'bg-danger'}"></span>
        <span>WhatsApp {appState.whatsapp ? 'connected' : 'disconnected'}</span>
      </div>
    {/if}
  </section>
  ...
  <section class="border border-border p-3">
    ...
    <div class="grid grid-cols-3 gap-2 text-xs">
      ...
      <div><span class="text-muted">Active</span><br><span class="text-lg text-accent">{appState.activeCount}</span></div>
